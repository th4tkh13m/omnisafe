...
class ReplayBuffer:
    def __init__(self,
        ...
        self.obs_shape = obs_space.shape # Size of individual frames.
        
        self.obs_dtype = torch.uint8 if pixel_obs else torch.float
        # Size of state given to network
        self.state_shape = [self.obs_shape[0] * history] # Channels or obs dim.
        if pixel_obs: self.state_shape += [self.obs_shape[1], self.obs_shape[2]] # Image size.
        self.num_channels = self.obs_shape[0] # Used to grab only the most recent obs (history) or channels.

        self.device = device

        # Store obs on GPU if they are sufficient small.
        memory, _ = torch.cuda.mem_get_info()
        obs_space_mem = np.prod((self.max_size, *self.obs_shape)) * 1 if pixel_obs else 4
        ard_space = self.max_size * (self._act_dim + 2) * 4
        if obs_space_mem + ard_space < memory:
            self.storage_device = self.device
        else:
            self.storage_device = torch.device('cpu')

        self.action_dim = self._act_dim
        ...
        self.action_reward_notdone = torch.zeros((self.max_size, self._act_dim + 2), device=self.device, dtype=torch.float)
